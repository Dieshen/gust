...
- **A compatibility corpus and golden output snapshots** (`gust-lang/tests/compat/`). Every `.gu` under a release directory must keep parsing and validating, and its generated Rust and Go must match a recorded golden. Corpus files are frozen after their release ships — rewriting one to satisfy a new rule would turn "1.0 source still compiles" into "source we were willing to change still compiles". Seeded with the standard library, the examples, the project template, and `regressions.gu`, a purpose-built file gathering the shapes that have actually broken.

- **A CI gate requiring a CHANGELOG entry** alongside any change to compiler sources, with a `no-changelog` label as the escape hatch. #120 and #121 shipped unannounced despite being authored, reviewed, and merged by people who knew about them.
...
- **Handlers may only call declared effects.** Gust has no function declarations, so a bare `helper(x)` named nothing the compiler knew — and both backends emitted it verbatim into generated code, resolving against whatever the surrounding module or package had in scope. `let x = exit(ctx.n);` reported "Check passed" and emitted `let _ = exit(n);`. This is the sandbox boundary, and it was open. Now a validator error, with a targeted diagnostic when the name is a declared effect called without `perform`. The [security guide](../guides/security.md) already claimed "there is no hidden call"; it is now enforced rather than asserted.

- **Unknown generic type constructors are rejected.** `Vec`, `Option`, and `Result` are the only ones any backend lowers. An unrecognised head passed straight through and the backends disagreed three different ways — Go emitted the non-existent `HashMap[K, V]`, Rust emitted an unimported `HashMap<K, V>`, JSON Schema emitted `"Unresolved generic type"`. Map- and set-shaped names get a diagnostic saying what to do instead. Closes #133.

- **Go: `goto` now ends the handler.** The Go backend lowered `goto` to a bare state assignment and kept going, so an early `goto` inside a bare `if` fell through into every later branch. The Rust backend has returned here since 0.4.0; Go was missed, and the asymmetry survived because nothing tested for it — `gust check` passed, the output compiled, and `go vet` was quiet, because the defect was purely behavioural. Two failure modes: a multi-target transition silently collapsed to its **last** declared target regardless of the condition, and a fall-through `goto` reading a source-state field dereferenced the pointer `clearStateData()` had already nulled. **This changes the runtime behaviour of Go machines that already compiled.** See [Upgrading 0.4 → 1.0](upgrading-0.4-to-1.0.md).
...
- **`gust-build` regenerates on content, not mtime.** Output was gated on the source being *newer* than its `.g.rs`; after a fresh clone every file carries the checkout timestamp, so a stale committed output was never rewritten. It now compares generated bytes. Identical output is still left untouched, so Cargo does not rebuild dependents needlessly.
...
- **`goto` now ends the handler.** It emitted a bare state assignment and execution continued, so an early `goto` inside an `if` fell through — the machine ended in whichever state the *last* assignment named, and a value moved into the abandoned state left the rest of the handler using a moved value (`E0382`). `goto` now returns. **This changes the runtime behaviour of machines that already compiled:** a handler that gotos `Early` when `n > 0` and otherwise falls to `Late` previously reached `Late` for every input.
- **`supervises` and `spawn` now generate a real contract.** `supervises` previously emitted nothing on the Rust backend and `spawn` emitted a future that discarded its arguments and returned `Ok(())` on both — no child was ever constructed. Each supervising machine now emits a runner trait (`{Machine}Supervision` in Rust, `{Machine}Children` in Go) with one method per child, plus a restart-strategy table. A host compiling against such a machine now has a trait to implement that did not exist before.
