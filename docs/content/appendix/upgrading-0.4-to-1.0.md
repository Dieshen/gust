...
## 2. Go: `goto` now ends the handler — this changes runtime behaviour
...
## 3. Handlers may only call declared effects
...
## 4. Unknown generic type constructors are rejected
...
## 5. Go builds refuse a `Result` error type Go cannot carry
...
## 6. `gust-build` validates, and regenerates on content
...
## 7. The `wasm` and `nostd` backends are removed
...
## 8. `--target ffi` requires `--unstable-ffi`
