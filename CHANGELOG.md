...
- **A compatibility corpus and golden output snapshots**
  (`gust-lang/tests/compat/`). Every `.gu` under a release directory must keep
  parsing and validating, and its generated Rust and Go must match a recorded
  golden. Corpus files are frozen after their release ships: rewriting one to
  satisfy a new rule would turn "1.0 source still compiles" into "source we were
  willing to change still compiles". Seeded with the standard library, the
  examples, the project template, and `regressions.gu` — a purpose-built file
  gathering the shapes that have actually broken. Reverting the `goto` fix was
  measured to move one golden before `regressions.gu` existed and two after.

- **A CI gate requiring a CHANGELOG entry** alongside any change to compiler
  sources, with a `no-changelog` label as the escape hatch. #120 and #121
  shipped unannounced despite being authored, reviewed, and merged by people who
  knew about them — intent was not the missing piece.
