...
pub enum {name}Error {{
    ...
    fn emit_transition_method(
        ...
        self.line(&format!(
            ...
            if handler.map(|h| h.is_async).unwrap_or(false) { "async " } else { "" },
            transition.name
        ));
        self.indent += 1;
        ...
        if let Some(handler) = handler {
            self.emit_block(&handler.body, state_enum, states, effects);
        } else {
    ...
    fn emit_block(
        ...
        for stmt in &block.statements {
            self.emit_statement(stmt, state_enum, states, effects);
        }
    }
    ...
    fn emit_statement(
        ...
        match stmt {
            ...
            Statement::If {
                ...
                self.indent += 1;
                self.emit_block(then_block, state_enum, states, effects);
                self.indent -= 1;
                if let Some(else_blk) = else_block {
                    self.line("} else {");
                    self.indent += 1;
                    self.emit_block(else_blk, state_enum, states, effects);
                    self.indent -= 1;
                }
                self.line("}");
            }
            ...
            Statement::Match { scrutinee, arms } => {
                ...
                for arm in arms {
                    ...
                    self.indent += 1;
                    self.emit_block(&arm.body, state_enum, states, effects);
                    self.indent -= 1;
                    self.line("}");
                }
                self.indent -= 1;
