...
impl RustCodegen {
    ...
    fn emit_prelude(&mut self) {
        ...
        // `use` emits nothing. It is a *Gust-level* import: it names a type
        // declared elsewhere so the validator will accept it, and the consumer's
        // build pipeline is responsible for putting that declaration in the same
        // module as the dependent machine.
        //
        // Until 1.0 a non-`std` path was passed through as a host `use`
        // statement, which made the keyword mean two different things and put a
        // `.gu` in reach of the host's namespace. Nothing could be called
        // through it — handlers may only call declared effects — so on Go it
        // produced an import that was always unused, and therefore output that
        // did not compile. Reserving the keyword for the module system planned
        // for 1.x also means its meaning does not change after 1.0.
        self.newline();
    }
